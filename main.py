...
async def load_browser_cookies():
    """
    Load cookies from the browser using browser-cookie3.
    Supports Chrome, Chromium, Firefox, Brave, Edge, Opera, Vivaldi, and Safari.
    
    Returns:
        tuple: (secure_1psid, secure_1psidts) or (None, None) if not found
    """
    try:
        ...
        for browser_func in [
            ...
            try:
                jar: CookieJar = browser_func(domain_name="google.com")
                if jar:
                    cookies = {cookie.name: cookie.value for cookie in jar}
                    
                    # Look for the required cookies
                    if "__Secure-1PSID" in cookies:
                        secure_1psid = cookies["__Secure-1PSID"]
                        secure_1psidts = cookies.get("__Secure-1PSIDTS", None)
                        
                        logger.success(f"Loaded cookies from {browser_func.__name__}")
                        return secure_1psid, secure_1psidts
                        
            except (bc3.BrowserCookieError, PermissionError, Exception) as e:
                logger.debug(f"Could not load cookies from {browser_func.__name__}: {e}")
                continue
        
        logger.warning("No cookies found in any browser")
        return None, None
        ...
        logger.error(f"Error loading browser cookies: {e}")
        return None, None
...
async def get_client():
    ...
    if client is None:
        try:
            ...
            # If no cookies configured, try to load from browser
            if not secure_1psid:
                if settings.gemini.use_browser_cookies:
                    logger.info("No manual cookies configured. Attempting to load from browser...")
                    secure_1psid, secure_1psidts = await load_browser_cookies()
                    
                    if not secure_1psid:
                        raise HTTPException(
                            status_code=503,
                            detail="Gemini authentication not configured. "
                                   "Please either:\n"
                                   "1. Set SECURE_1PSID and SECURE_1PSIDTS environment variables, or\n"
                                   "2. Set USE_BROWSER_COOKIES=true and login to Gemini in your browser"
                        )
                    else:
                        logger.info("Successfully loaded cookies from browser")
                else:
                    raise HTTPException(
                        status_code=503,
                        detail="Gemini authentication not configured. "
                               "Please set SECURE_1PSID environment variable or set USE_BROWSER_COOKIES=true"
                    )
